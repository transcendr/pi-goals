# 01-protocol-read — workflow references read

## Step actually performed

Read the governing project and workflow instructions before/while creating the issue doc.
...
```toon
workflow_requirements[5]{id,requirement}:
  wf1,"issue doc must become canonical planning doc"
  wf2,"research findings must be grounded in live code and written back into issue"
  wf3,"execution-ready requires meaningful choices locked or explicitly rejected"
  wf4,"proof threat model must make false-green risks explicit"
  wf5,"required_proofs[] rows must be concrete and aligned to the locked design"
```
