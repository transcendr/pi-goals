# 01 — Protocol read

Freshly read or present in current context:

- `~/.codex/feature-workflow-pipelines/SKILL.md`
- `~/.codex/feature-workflow-pipelines/references/canonical-issue-docs-and-toon-synthesis.md`
- `~/.codex/feature-workflow-pipelines/references/research-pass.md`
- `~/.codex/feature-workflow-pipelines/references/design-landscape-exploration-and-choice-locking.md`
- `~/.codex/feature-workflow-pipelines/references/toon-for-issues-and-execution-planning.md`

Extracted requirements:

- Keep the issue doc canonical.
- Ground planning in inspected code, docs, and recent issue evidence.
- Lock meaningful design choices before marking execution-ready.
- Include proof threat model before required proof rows.
- Use valid TOON for `required_proofs[]` when the issue should run through proof-driven execution.
- Create visible workflow artifacts `00` through `06` and `raw/commands.log`.
- Verify artifact visibility with `git status --short --untracked-files=all` and `git check-ignore -v`.
