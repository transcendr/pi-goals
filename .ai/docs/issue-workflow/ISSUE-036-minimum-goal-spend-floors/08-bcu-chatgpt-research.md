# 08 — BCU + ChatGPT research evidence

## Context

This artifact records the BCU-mediated ChatGPT research pass folded into the active ISSUE-036 planning goal. The monitor steered away from BCU-log fixation and toward extracting current ChatGPT evidence into the issue workflow; this file is that extraction and synthesis.

## BCU state and extraction evidence

- BCU runtime was started successfully by opening the installed `~/Applications/BackgroundComputerUse.app` and verifying `bootstrap_ready: true` / `permissions_ready: true`.
- Safari window `w_1X3V3EQH` was the ChatGPT conversation.
- A screenshot showed ChatGPT at the bottom of the response, with no thinking/generating indicator.
- The visible down-arrow was clicked to jump to the response bottom.
- The visible Copy response button was clicked by screenshot coordinates; `pbpaste` captured the response into `raw/chatgpt-response-round1.txt`.
- Raw extracted response size: 17216 characters.

## Important caveat

The first ChatGPT prompt was too weak because it did not include enough pi-goals project context. The user corrected this. Treat the response as a useful external critique, not as authoritative repo-grounded truth. We should only carry forward suggestions that match the local code/research facts already recorded in `02-grounded-research.md`.

## Extracted design findings to carry forward

1. Reframe the feature internally away from “minimum spend” and toward “premature completion prevention” / “completion floor”. The floor is not a quota target; it is a completion-quality skepticism gate.
2. Do not implement the gate as `floor unmet -> keep working until floor met`. That directly creates churn.
3. Make completion go through a named completion-gate helper/module instead of mutating state directly inside `update_goal`.
4. Represent additional floor work as bounded work cards, not loose prompt vibes. The existing issue’s `FLOOR_VALUE_PASS_CATALOG` maps well to this; rename or structure rows as `FloorWorkCard` if helpful.
5. Track floor-work history to avoid repetition: at least last card id and completed card ids; consider evidence hashes/notes for completed floor work.
6. Add an explicit no-more-valuable-work escape reason. Floors need an honorable autonomous escape hatch when all safe valuable work has been exhausted; otherwise the feature can manufacture work.
7. Treat “ask user what next?” as policy-controlled and default-denied, matching owner feedback.
8. Make the monitor judge whether floor work produced a new objective-linked improvement, not merely whether tokens/time increased.
9. Add a `floor_quality_exhausted` or equivalent state/reason after repeated bad/no-evidence floor work so the system stops forcing churn.
10. Max budgets must override min floors; a floor must never defeat a hard stop.
11. Validation should be adversarial, including no-valuable-work, repeated-card, resume/compaction, max-budget conflict, and monitor steer cases.

## Response excerpt

The copied response included this useful mantra:

> Minimum floors are not quotas. They are completion skepticism. When skeptical, steer toward the next most valuable bounded proof-producing action. When no such action exists, allow completion rather than manufacture work.

## Full raw response

See `raw/chatgpt-response-round1.txt` for the complete copied ChatGPT response.

## Selected raw excerpts

```text
Round 1 critique: the feature should **not** be “minimum spend.” That framing invites token laundering. It should be **“premature completion prevention with useful-work steering.”** The floor is only a guardrail that says: “before declaring done, make one more high-value pass from a reusable work catalog unless there is a real blocker, max budget conflict, or no valuable autonomous work remains.”

Pi has the right extension hooks for this: extensions can register model-callable tools, intercept lifecycle/model/tool events, persist session state, and send/inject messages; current goal packages already expose `/goal`, `get_goal`, `create_goal`, `update_goal`, continuation prompts, and token/time budget surfaces. ([pi.dev][1])

## 1. Main design critique

The dangerous implementation is:

> floor unmet → reject completion → “keep working until floor is met”

That creates churn.

The better implementation is:

> floor unmet → completion gate asks catalog for the best next valuable work card → agent must do that work with evidence → churn monitor judges whether the floor work is real → completion allowed once floors met or valuable work is exhausted.

So the invariant should be:

> A floor can delay completion, but it cannot justify unrelated work.

## 2. Suggested module split

Use a small set of explicit modules instead of burying this inside `update_goal`.
...
`usage-accounting.ts`
Tracks active elapsed time and token usage. Use Pi’s context usage where available, but keep your own goal-start snapshots so the floor is relative to the goal, not the whole session. Pi exposes `ctx.getContextUsage()` for current model context/token accounting. ([pi.dev][1])

`completion-gate.ts`
Owns the decision: complete, steer, ask, pause, or hard-stop.

`catalog.ts`
Reusable catalog of valuable follow-up work patterns.

`selector.ts`
Chooses one card based on objective, changed files, prior actions, evidence, floor deficit, and churn history.

`churn-monitor`
Existing intelligent churn monitor becomes the quality layer. It does not merely say “agent is looping”; it says “this attempted floor work is not advancing the objective.”

## 3. API shape

Extend goal creation:
...
type CreateGoalInput = {
  ...
  floor_policy?: {
    ...
    user_fallback?: "never" | "objective_allows" | "always";
    no_value_escape?: "allow_complete" | "pause_goal" | "ask_user";
    max_floor_steers?: number;
    max_churn_steers?: number;
  };
};
` ` `
...
type GoalFloorState = {
  ...
  token_floor_met: boolean;
  time_floor_met: boolean;

  token_deficit: number;
  time_deficit_seconds: number;

  floor_steer_count: number;
  churn_steer_count: number;

  last_floor_card_id?: string;
  completed_floor_card_ids: string[];
...
## Concrete improvements excerpt

```text
## 11. Concrete design improvements to make now

1. Rename internal concept from `minimum spend` to `completion floor` or `premature wrap-up floor`.
2. Make `update_goal(status: complete)` pass through `completion-gate.ts`; don’t mutate goal state directly.
3. Add a reusable `FloorWorkCard` catalog before adding prompt text.
4. Store `completed_floor_card_ids` and `last_floor_card_id`; repetition is the root of floor slop.
5. Add `no_more_valuable_work_reason`; floors need an honorable escape hatch.
6. Treat “ask user what next?” as a policy-controlled action, default denied.
7. Make the churn monitor floor-aware: it should judge whether floor work produced new objective evidence.
8. Add `floor_quality_exhausted`; after repeated bad floor work, stop enforcing the floor.
9. Ensure max budgets override min floors. A min floor must never defeat a hard stop.
10. Validate with adversarial probes, not just happy-path tests.

The core mantra I’d put in the implementation comments:

` ` `text
Minimum floors are not quotas.
They are completion skepticism.
When skeptical, steer toward the next most valuable bounded proof-producing action.
When no such action exists, allow completion rather than manufacture work.
` ` `

[1]: https://pi.dev/docs/latest/extensions "Pi Coding Agent"
[2]: https://pi.dev/packages/pi-codex-goal "Pi Coding Agent"

```


## Round 2 grounded prompt submitted

Monitor steering corrected the workflow: stop optimizing ChatGPT copy/button mechanics and first preserve the grounded prompt in the research artifact. The prompt below was built from the public package links, current ISSUE-036 design, and code excerpts from the actual `.pi/extensions/goal` modules.

Prompt saved separately at `raw/chatgpt-round2-grounded-prompt.txt`.

<details><summary>Grounded ChatGPT prompt</summary>

```text
You are reviewing a real public TypeScript Pi extension feature design. This is not generic brainstorming.

Public project:
- GitHub: https://github.com/transcendr/pi-goals
- npm: https://www.npmjs.com/package/pi-goals

Task: perform a grounded design review for ISSUE-036 "Minimum goal spend floors". The feature adds optional `min_tokens_before_wrap_up` and `min_time_seconds_before_wrap_up` to pi-goals. These are completion floors / premature wrap-up floors, not quotas. They prevent normal completion/wrap-up before minimum goal-directed work, but max budgets/hard stops must still override. The most important part is autonomous useful-work steering: if the floor is unmet and the agent tries to finish, it must choose valuable extra work, not filler; it must not ask the user unless the goal objective explicitly allows it or a separate safety/destructive-action boundary requires it. The existing churn monitor should become the quality backstop.

I previously sent you too little context. Below are the actual current issue design and relevant code excerpts. Please produce a high-value ROUND 2 design review:

Output requested:
1. Identify concrete gaps or risky ambiguities in the current issue design.
2. Propose exact TypeScript types/functions/modules to add, with names matching this repo's style.
3. Refine the completion-gate + floor-steering design, including how to handle "no valuable autonomous work remains" without making floors meaningless.
4. Refine the floor-aware churn monitor design: report fields, pattern names, XML decision examples, scoring/evidence deltas, and how it should interact with telemetry.
5. Specify adversarial validation probes that would fail on generic prompt-only or schema-only implementations.
6. Call out anything in the proposed design that is overbuilt for first release and should be deferred.

Do not give generic advice. Use the code/module context. Be concise but concrete.

# Current ISSUE-036 excerpt
...
- no queue-complete steering is emitted from the rejected completion attempt.

### Autonomous floor-value continuation design

The qualitative steering must be implemented as concrete shared logic, not only hand-written prompt prose. Add two narrow design surfaces:

1. `.pi/extensions/goal/completion-gate.ts` owns completion decisions and is called by `update_goal(status:"complete")` before any state mutation.
2. `.pi/extensions/goal/floor-steering.ts` owns catalog-backed floor work selection and prompt/refusal text.
...
- `type CompletionDecision = { kind: "allow" } | { kind: "defer_and_steer"; card: FloorWorkCard; floor: FloorEvaluation } | { kind: "allow_with_reason"; reason: NoMoreValuableWorkReason | "max_budget_requires_wrap_up" }`;
- `decideGoalCompletion(goal, telemetry, floor, monitor): CompletionDecision`;
- `buildCompletionGateResult(decision)` for tool details and prompt text.
...
Important: when useful autonomous work is exhausted, the system should allow completion with a recorded `noMoreValuableWorkReason` / `floorQualityState: "exhausted"` rather than manufacture work just to satisfy a numeric floor.
...
Update tool parameters accept `null` to remove a configured floor, matching existing max-budget edit conventions.
...
- [ ] Add optional minimum floor fields to `GoalState`, queued goal state, tool detail types, a
` ` `
...
181: }
182: 
183: function updateGoalGuidelines(): string[] {
184: 	return [
185: 		"Use update_goal with status complete only after verifying the goal objective is actually achieved.",
186: 		"Use update_goal for objective or budget edits only when the user explicitly asks to modify the persistent goal.",
187: 		"Use status paused or active only when the user explicitly asks to pause or resume the goal.",
188: 		"Do not infer goal edits from ordinary task discussion.",
189: 		"Use clear_goal, not update_goal, when the user asks to clear/remove the current goal.",
190: 	];
191: }
192: 
193: type CreateGoalInput = {
194: 	objective: string;
195: 	token_budget?: number;
196: 	time_budget_seconds?: number;
197: };
198: 
199: type CreateGoalFromTemplateInput = {
200: 	template: string;
201: 	flags?: Record<string, string>;
202: 	args?: string;
203: 	token_budget?: number;
204: 	time_budget_seconds?: number;
205: };
206: 
207: type UpdateGoalInput = {
208: 	status?: string;
209: 	objective?: string;
210: 	token_budget?: number | null;
211: 	time_budget_seconds?: number | null;
212: };
213: 
214: type GoalUpdateResult = { ok: true; goal: GoalState; prefix: string; budgetChanged?: boolean } | { ok: false; error: string };
215: 
216: function createGoalFromTool(pi: ExtensionAPI, runtime: GoalToolRuntime, params: CreateGoalInput, ctx: ExtensionContext) {
217: 	return createGoalWithPolicy(pi, runtime, params, ctx, { replaceCompleted: false });
218: }
219: 
220: function createGoalFromTemplateTool(pi: ExtensionAPI, runtime: GoalToolRuntime, params: CreateGoalFromTemplateInput, ctx: ExtensionContext) {
221: 	const resolved = resolveGoalTemplateInvocationArgs(params.template, params.args ?? "", params.flags ?? {});
222: 	if (!resolved.ok) return "notTemplate" in resolved ? errorResult(`Unknown goal template '${params.template}'.`) : errorResult(resolved.error);
223: 	const created = createGoalWithPolicy(pi, runtime, { objective: resolved.template.objective, token_budget: params.token_budget, time_budget_seconds: params.time_budget_seconds }, ctx, { replaceCompleted: true });
224: 	if (!created.details.error) created.details.resolved_template = { name: resolved.template.name, path: resolved.template.path };
225: 	return created;
226: }
227: 
228: function createGoalWithPolicy(
229: 	pi: ExtensionAPI,
230: 	runtime: GoalToolRuntime,
231: 	params: CreateGoalInput,
232: 	ctx: ExtensionContext,
233: 	policy: { replaceCompleted: boolean },
234: ) {
235: 	const current = getGoal();
236: 	if (current && (!policy.replaceCompleted || current.status !== "complete")) {
237: 		return errorResult("A goal already exists. Use clear_goal or ask the user before replacing it.");
238: 	}
239: 	const validation = validateObjective(params.objective);
240: 	if (!validation.ok) return errorResult(validation.hint ? `${validation.message} ${validation.hint}` : validation.message);
241: 	const budgetError = validateBudgets(params.token_budget, params.time_budget_seconds);
242: 	if (budgetError) return errorResult(budgetError);
243: 	const goal = createGoalState(validation.objective, params.token_budget, params.time_budget_seconds);
244: 	const telemetry = createTelemetry(goal.goalId, goal.createdAt);
245: 	persistSetGoal(pi, goal, telemetry, "tool");
246: 	syncGoalUi(ctx, goal);
247: 	runtime.scheduleMonitor?.(ctx);
248: 	return resultForGoal(goal, telemetry, current?.status === "complete" ? "Goal created; replaced completed goal." : "Goal created.");
249: }
250: 
251: function updateGoalFromTool(pi: ExtensionAPI, runtime: GoalToolRuntime, params: UpdateGoalInput, ctx: ExtensionContext) {
252: 	const goal = getGoal();
253: 	if (!goal) return errorResult("No goal exists to update.");
254: 	const update = buildGoalUpdate(goal, params);
255: 	if (!update.ok) return errorResult(update.error);
256: 	// Refuse to reactivate a goal whose budgets are still exhausted.
257: 	if (update.goal.status === "active" && !canActivateGoal(update.goal)) {
258: 		const reason = isBudgetExhausted(update.goal);
259: 		const resource = reason === "tokenBudget" ? "token" : reason === "timeBudget" ? "time" : "budget";
260: 		return errorResult(`Cannot resume: ${resource} budget is still exhausted. Raise the budget or use clear_goal before resuming.`);
261: 	}
262: 	if (update.goal.status !== "active") {
263: 		runtime.cancelContinuation?.(goal.goalId, "tool-status");
264: 		runtime.cancelMonitor?.(goal.goalId, "tool-status");
265: 	}
266: 	// When a budget edit transitions active -> budgetLimited, schedule budget-limit wrap-up once.
267: 	budgetEditWrapUpIfNeeded(pi, ctx, runtime, goal, update.goal);
268: 	persistUpdateGoal(pi, update.goal, telemetryForUpdate(update.goal), "tool");
269: 	syncGoalUi(ctx, update.goal);
270: 	if (goal.status !== "active" && update.goal.status === "active") {
271: 		runtime.scheduleMonitor?.(ctx);
272: 		runtime.scheduleContinuation?.(ctx, "resumed");
273: 	}
274: 	return resultForGoal(update.goal, getTelemetry(), update.prefix);
275: }
276: 
277: function validateBudgets(tokenBudget?: number, timeBudgetSeconds?: number): string | undefined {
278: 	if (tokenBudget !== undefined && (!Number.isInteger(tokenBudget) || tokenBudget <= 0)) return "token_budget must be a positive integer when provided.";
279: 	if (timeBudgetSeconds !== undefined && (!Number.isInteger(timeBudgetSeconds) || timeBudgetSeconds <= 0)) {
280: 		return "time_budget_seconds must be a positive integer when provided.";
281: 	}
282: 	return undefined;
283: }
284: 
285: function buildGoalUpdate(goal: GoalState, params: UpdateGoalInput): GoalUpdateResult {
286: 	let next: GoalState = { ...goal };
287: 	const changes: string[] = [];
288: 	const objectiveResult = applyObjectiveUpdate(next, params.objective, changes);
289: 	if (!objectiveResult.ok) return objectiveResult;
290: 	next = objectiveResult.goal;
291: 	const budgetResult = applyBudgetUpdates(next, params, changes);
292: 	if (!budgetResult.ok) return budgetResult;
293: 	next = budgetResult.budgetChanged && params.status === undefined ? recomputeStatusAfterBudgetEdit(budgetResult.goal) : budgetResult.goal;
294: 	if (params.status !== undefined) {
295: 		const status = parseToolStatus(params.status);
296: 		if (!status) return { ok: false, error: "status must be active, paused, or complete when provided." };
297: 		next = { ...next, status };
298: 		changes.push(`status ${status}`);
299: 	}
300: 	if (changes.length === 0) return { ok: false, error: "No goal updates were provided." };
301: 	return { ok: true, goal: { ...next, updatedAt: Date.now() }, prefix: next.status === "complete" ? "Goal achieved." : `Goal updated: ${changes.join(", ")}.` };
302: }
303: 
304: function applyObjectiveUpdate(goal: GoalState, objective: string | undefined, changes: string[]): GoalUpdateResult {
305: 	if (objective === undefined) return { ok: true, goal, prefix: "" };
306: 	const validation = validateObjective(objective);
307: 	if (!validation.ok) return { ok: false, error: validation.hint ? `${validation.message} ${validation.hint}` : validation.message };
308: 	changes.push("objective");
309: 	return { ok: true, goal: { ...goal, objective: validation.objective }, prefix: "" };
310: }
311: 
312: function applyBudgetUpdates(goal: GoalState, params: UpdateGoalInput, changes: string[]): GoalUpdateResult {
313: 	let next = goal;
314: 	let budgetChanged = false;
315: 	if (params.token_budget !== undefined) {
316: 		if (params.token_budget !== null && (!Number.isInteger(params.token_budget) || params.token_budget <= 0)) return { ok: false, error: "token_budget must be a positive integer or null when provided." };
317: 		next = { ...next, tokenBudget: params.token_budget === null ? undefined : params.token_budget };
318: 		changes.push("token budget");
319: 		budgetChanged = true;
320: 	}
321: 	if (params.time_budget_seconds !== undefined) {
322: 		if (params.time_budget_seconds !== null && (!Number.isInteger(params.time_budget_seconds) || params.time_budget_seconds <= 0)) return { ok: false, error: "time_budget_seconds must be a positive integer or null when provided." };
323: 		next = { ...next, timeBudgetSeconds: params.time_budget_seconds === null ? undefined : params.time_budget_seconds };
324: 		changes.push("time budget");
325: 		budgetChanged = true;
326: 	}
327: 	return { ok: true, goal: next, prefix: "", budgetChanged };
328: }
329: 
330: function budgetEditWrapUpIfNeeded(pi: ExtensionAPI, ctx: ExtensionContext, runtime: GoalToolRuntime, previousGoal: GoalState, updatedGoal: GoalState) {
331: 	// Only schedule wrap-up when an active goal transitions to budgetLimited due to a budget edit.
332: 	if (previousGoal.status === "active" && updatedGoal.status === "budgetLimited" && isBudgetExhausted(updatedGoal)) {
333: 		runtime.scheduleBudgetLimitWrapUp?.(ctx, updatedGoal);
334: 	}
335: }
336: 
337: function recomputeStatusAfterBudgetEdit(goal: GoalState): GoalState {
338: 	if (goal.status !== "active" && goal.status !== "budgetLimited") return goal;
339: 	return isBudgetExhausted(goal) ? { ...goal, status: "budgetLimited" } : { ...goal, status: "active" };
340: }
341: 
342: function telemetryForUpdate(goal: GoalState): GoalTelemetrySnapshot | null {
343: 	const reason = isBudgetExhausted(goal);
344: 	if (goal.status === "budgetLimited" && reason) return noteBudgetLimit(getTelemetry(), reason);
345: 	return getTelemetry();
346: }
347: 
348: function parseToolStatus(status: string): GoalStatus | undefined {
349: 	if (status === "active" || status === "paused" || status === "complete") return status;
350: 	return undefined;
351: }
352: 
353: function resultForGoal(goal: GoalState | null, telemetry: GoalTelemetrySnapshot | null, prefix?: string) {
354: 	const details: ToolDetails = { goal, telemetry, remaining_tokens: remainingTokens(goal), remaining_time_seconds: remainingTime(goal) };
355: 	if (goal?.status === "complete" && (goal.tokenBudget !== undefined || goal.timeBudgetSeconds !== undefined)) {
356: 		details.completion_budget_report = `Goal achieved. Report final budget usage to the user: tokens used: ${goal.tokensUsed}${goal.tokenBudget === undefined ? "" : ` of ${goal.tokenBudget}`}; time used: ${goal.timeUsedSeconds}${goal.timeBudgetSeconds === undefined ? "" : ` of ${goal.timeBudgetSeconds}`} seconds.`;
357: 	}
358: 	return { content: [{ type: "text" as const, text: `${prefix ? `${prefix}\n` : ""}${formatToolGoal(goal)}` }], details };
359: }
360: 
361: function resultForTemplates(templates: ReturnType<typeof listGoalTemplateMetadata>) {
362: 	const lines = templates.map((template) => {
363: 		const aliases = template.aliases.length ? ` aliases=${template.aliases.join(",")}` : "";
364: 		const placeholders = template.requiredPlaceholders.length ? ` placeholders=${template.requiredPlaceholders.join(",")}` : " placeholders=none";
365: 		return `${template.name}${aliases}${placeholders} path=${template.path}${template.description ? ` — ${template.description}` : ""}`;
366: 	});
367: 	return { content: [{ type: "text" as const, text: lines.length ? lines.join("\n") : "No reusable goal templates found." }], details: { goal: getGoal(), telemetry: getTelemetry(), templates } as ToolDetails };
368: }
369: 
370: function errorResult(error: string) {
371: 	return { content: [{ type: "text" as const, text: `Error: ${error}` }], details: { goal: getGoal(), telemetry: getTelemetry(), error } as ToolDetails };
372: }
373: 
374: function remainingTokens(goal: GoalState | null): number | undefined {
375: 	return goal?.tokenBudget === undefined ? undefined : Math.max(0, goal.tokenBudget - goal.tokensUsed);
376: }
377: 
378: function remainingTime(goal: GoalState | null): number | undefined {
379: 	return goal?.timeBudgetSeconds === undefined ? undefined : Math.max(0, goal.timeBudgetSeconds - goal.timeUsedSeconds);
380: }
381: 
382: function formatToolGoal(goal: GoalState | null): string {
383: 	if (!goal) return "No goal is currently set.";
384: 	const tokenBudget = goal.tokenBudget === undefined ? "none" : `${goal.tokenBudget} (${remainingTokens(goal)} remaining)`;
385: 	const timeBudget = goal.timeBudgetSeconds === undefined ? "none" : `${formatElapsed(goal.timeBudgetSeconds)} (${remainingTime(goal)} seconds remaining)`;
386: 	return [
387: 		`Goal: ${goal.status}`,
388: 		`Objective: ${goal.objective}`,
389: 		`Time used: ${goal.timeUsedSeconds} seconds`,
390: 		`Tokens used: ${goal.tokensUsed}`,
` ` `
...
## Round 2 grounded response captured

The grounded Round 2 response was captured with ChatGPT's visible **Copy response** button after generation completed.

- Raw response: `raw/chatgpt-response-round2-grounded.txt`
- Raw prompt: `raw/chatgpt-round2-grounded-prompt.txt`
- Captured response size: ~29.5K characters

## Round 2 concrete findings to carry forward

Unlike Round 1, Round 2 reviewed the actual issue/design context and local module excerpts. The useful deltas are implementation-specific:

1. **Make `noMoreValuableWorkReason` an outcome, not permission.** A worker may report that no safe valuable work remains, but first-attempt `update_goal(status:"complete", no_more_valuable_work_reason: ...)` must not self-authorize a floor bypass. The completion gate should honor that reason only after telemetry/monitor evidence such as `floorQualityState: "exhausted"`, max-budget exhaustion, an exhausted catalog selector after concrete floor passes, or explicit user floor removal/change.
2. **Insert the completion gate before destructive completion side effects.** In `updateGoalFromTool`, build/validate the candidate update first, then run `decideGoalCompletion` before canceling continuations/monitors or persisting `status: "complete"`.
3. **Deferral should be a normal tool result, not an error-shaped failure.** The message should begin with `Completion deferred by goal floor. The goal remains active.` and return structured details including `completion_blocked_by_floor: true`, floor deltas, and `next_floor_pass`.
4. **Soft final-answer wrap-up is a separate bypass risk.** Existing `agentEnd` continuation suppression can swallow floor steering when the model simply says “Done” without calling `update_goal`. The lifecycle check should treat `agentEnd + floor unmet` as eligible for floor-aware continuation unless floor quality is exhausted or a steer limit is reached.
5. **Persist explicit telemetry mutation points.** Add helpers such as `noteFloorCompletionDeferred`, `noteProductiveFloorWork`, `noteFloorChurnSteer`, and `noteFloorQualityExhausted`. Without these, the selector cannot avoid repeated cards and the monitor cannot safely trigger the no-value escape.
6. **Keep user fallback detection narrow.** Add a conservative `objectiveAllowsUserFloorFallback(objective)` helper with explicit phrases like “ask me”, “confirm with me”, “wait for my decision”, “let me choose”, or “before choosing”. Do not infer permission from vague words like “discuss”, “explore”, or “recommend”.
7. **Add a small pure floor helper module.** Round 2 recommends `.pi/extensions/goal/floor.ts` for `evaluateCompletionFloor`, `isCompletionFloorUnmet`, and `validateBudgetFloorConfig`, keeping max-budget caps in `budget.ts` authoritative.
8. **Use the current goal for floor math.** `decideGoalCompletion` should evaluate floors against `currentGoal`, not a candidate already mutated to `status: "complete"`.
9. **Bound the no-value escape.** First require at least one proof-producing floor pass or monitor-backed churn evidence. A helper like `canAllowNoValuableWorkEscape` can require `floorQualityState === "exhausted"`, several completed catalog cards, or repeated steer+churn counts before allowing `no_safe_autonomous_work`.
10. **Prefer deterministic catalog selection for release one.** Do not add an LLM selector or plugin framework. A static `FLOOR_VALUE_PASS_CATALOG` plus objective keyword ranking and completed/last-card penalties is enough.
11. **Make the floor-aware monitor mutate telemetry.** Add floor report history fields, stable patterns including `floor_quality_exhausted`, and map monitor decisions to telemetry updates after parsing each monitor report.
12. **Add adversarial probes beyond the happy path.** Include fake no-more-work bypass, soft final answer, unauthorized “what else should I do?”, quota filler, repeated same pass, productive floor deepening, queue/template propagation, and live-probe cases.

## Round 2 response excerpt

```text
The highest-leverage correction: make noMoreValuableWorkReason a recorded outcome, not a model-granted permission. The floor gate should be skeptical, but not stupid: first it demands one bounded proof-producing value pass; after repeated no-evidence attempts, the churn monitor is allowed to say “stop, this floor is now creating slop.”
```

## Round 3 execution-risk research captured

The second folded BCU research pass used a different angle: maintainer/test-engineer implementation risk rather than product design. Two follow-ups were submitted in the same ChatGPT conversation:

- Round 3A prompt: `raw/chatgpt-round3-execution-angle-prompt.txt`
- Round 3A copied response: `raw/chatgpt-response-round3-execution-angle.txt`
- Round 3B prompt: `raw/chatgpt-round3b-adversarial-followup-prompt.txt`
- Round 3B current page text extraction: `raw/chatgpt-response-round3b-adversarial-current-text.txt`

A churn-monitor steer explicitly warned to stop repeated screenshot/page polling and extract the available answer once. I followed that by saving the current page text from BCU into the raw Round 3B evidence file and proceeding from the captured research.

## Round 3 concrete findings to carry forward

1. **Patch order matters.** Add pure type/floor helpers first, then state propagation, validation/formatting, `floor-steering.ts`, `completion-gate.ts`, continuation lifecycle, monitor telemetry integration, and only then UI/README/live probes. This avoids schema/prompt false greens before behavior is wired.
2. **Gate insertion must be exact.** In `tools.ts`, `decideGoalCompletion(...)` must run after `buildGoalUpdate(...)` and active-resume budget checks but before `cancelContinuation`, `cancelMonitor`, `persistUpdateGoal(status:"complete")`, or queue-complete steering.
3. **Same-call floor removal plus completion is not a first-release escape hatch.** A single `update_goal` call must not both lower/remove an unmet floor and set `status:"complete"`; first release requires two calls so the gate evaluates current persisted floors.
4. **Use an options object for `createGoalState`.** Adding optional positional floor args after existing budget args is a silent integration footgun across create/template/queue paths.
5. **Put shared floor unions in `types.ts`.** `FloorValuePassId`, `FloorQualityState`, and `NoMoreValuableWorkReason` should live in shared types to avoid importing `floor-steering.ts` into `types.ts` or telemetry code.
6. **Add `PiGoalEventReason: "floor"`.** Telemetry persistence for floor events should not require casts.
7. **Strict tool input, defensive replay.** Tool schemas should reject malformed floor fields, but replay/loading should treat missing or malformed optional historical floor fields as absent instead of crashing state reconstruction.
8. **Budget-limited prompts stay floor-free.** Max-budget wrap-up must not ask for floor-value passes; max budgets dominate in the completion gate, continuation scheduling, and prompt building.
9. **Monitor floor telemetry needs stale guards.** Apply floor-pattern telemetry mutations only when the report goal id still matches the current active goal and `floor.completionBlockedByFloor` is true.
10. **Agent-end suppression must be floor-aware.** Existing no-tool auto-turn suppression should not swallow floor-unmet steering when the goal is active, budget is not exhausted, and floor quality is not exhausted.
11. **Floor history must be bounded/default-safe.** `completedFloorCardIds` should be unique and capped to catalog size or a fixed small cap; old telemetry should default to empty arrays/counters and `inactive` quality.
12. **Queue surfaces must be enumerated.** Probe `create_goal`, `create_goal_from_template`, `enqueue_goal`, queued persisted metadata, queue display, and `start_queued_goal` so floors cannot silently drop outside direct create.

## Round 3 maintainer invariant excerpt

```text
An unmet completion floor may block completion, but it must never cancel the active goal machinery, never override max-budget safety, and never be bypassed by same-call self-declared “no more valuable work.”
```

Round 3B produced a final invariant list worth carrying into the issue doc:
...
- A single update_goal call must not both remove/lower an unmet floor and complete the goal.
- noMoreValuableWorkReason is an internal recorded outcome, not a first-release model-granted bypass.
- An unmet-floor completion attempt rejects the completion mutation but returns a successful structured deferral result, not a generic error.
...
- createGoalState must use a named options object for max budgets and min floors.
