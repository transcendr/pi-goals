...
let runner = createContextResetActionRunner({ postCompletionActions:true, contextReset:false });
...
runner = createContextResetActionRunner({ postCompletionActions:true, contextReset:true });
result = await runner.run({ goal:{ goalId:'g1' }, action:{ id:'a1', type:'context.reset', mode:'clear', status:'pending' }, ctx:{} });
...
assert('available context reset runs adapter', result.ok && result.status === 'done' && navigated);
