...
assert('flags default on', getGoalFeatureFlags({}).postCompletionActions === true && getGoalFeatureFlags({}).contextReset === true);
for (const value of ['0','false','no','off','FALSE']) {
  ...
  assert(`disabled value ${value}`, flags.postCompletionActions === false && flags.contextReset === false);
}
assert('unknown values stay enabled', getGoalFeatureFlags({ PI_GOAL_POST_COMPLETION_ACTIONS:'typo', PI_GOAL_CONTEXT_RESET:'1' }).postCompletionActions === true);
