...
assert('lifecycle subscribes session_before_compact', /pi\.on\("session_before_compact", \(\) => \{ beginGoalCompaction\(pi\); \}\)/.test(lifecycle));
...
assert('finishGoalCompaction schedules compacted continuation for active goal', /scheduleMaybeContinueGoal\(pi, ctx, "compacted"\);/.test(continuation));
